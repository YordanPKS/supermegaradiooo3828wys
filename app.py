...
# Configuración optimizada para SA-MP Mobile
MUSIC_FOLDER = '/musica'
CHUNK_SIZE = 1024 * 16  # 16KB chunks
BUFFER_SIZE = 1024 * 1024 * 4  # 4MB buffer (reducido para móviles)
FIXED_BITRATE = 128000  # 128 kbps (optimizado para móviles)
...
def update_playlist():
    ...
    try:
        ...
        with state.lock:
            ...
            # Resetear índice si no es válido
...
def play_song(song_number):
    """Reproduce canción con verificación robusta"""
...
def broadcaster():
    ...
    state.active = True
    logger.info("Iniciando transmisión...")
    ...
    while state.active:
        try:
            ...
            logger.info(f"▶️ Reproduciendo: {song_name} (#{state.song_index + 1})")
            
            # Transmisión con control de tiempo preciso
            byte_counter = 0
        ...
        while state.active:
            with state.lock:
                ...
                if available >= CHUNK_SIZE:
                    ...
                    yield chunk
                    time.sleep(0.001)  # Ajuste fino para móviles
                else:
                    time.sleep(0.01)
...
def play_song_endpoint(song_number):
    ...
    if success:
        return jsonify({
            "status": "success",
            "song": state.song_map.get(index, f"Canción #{song_number}")
        })
...
if __name__ == '__main__':
    ...
    logger.info("=" * 50)
    logger.info(f"🎧  Radio SA-MP Mobile Iniciada  🎧")
    logger.info(f"📂 Música en: {MUSIC_FOLDER}")
    logger.info(f"🎵 Canciones: {len(state.playlist)}")
    logger.info(f"🌐 Dirección local: http://{IP}:{PORT}")
    logger.info("=" * 50)
